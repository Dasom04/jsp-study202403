...
<%--
    // 디스패처에 의해 request 객체가 이쪽으로 전달됐을 것입니다.
    // request에 담겨잇는 댄서 리스트를 꺼내는 메서드.
    List<Dancer> dancerList = (List<Dancer>) request.getAttribute("dl");

    // MVC 구조에서는 View 역할을 하는 jsp쪽에서 자바코드를 최소화 하기로 약속함.
    // el을 통해서 request에 있는 dl이라는 이름의 데이터를 깨내 쓰도록 하자.
--%>
...
<body>
    ...
    <ul id="dancer-list">
        ...
        <%--
            c:forEach 태그를 이용하여 향상 for문을 구혀하는 법.
            제어변수를 선언하시고, items 속성에 반복문을 돌리고자 하는 걸렉션 자료형을 세팅.
            request 객체 내에 있는 dl이라는 이름의 데이터를 el을 사용하여 꺼낼 수 있다.
            ${requestScope.dl} -> 요게 원형인데, Scope를 언급하지 않으면 dl이라는 이름의 데이터를
            전달된 모든 객체에서 뒤녀져서 꺼내 옵니다.
        --%>
        <c:forEach var="d" items="${dl}">
            <li>
                <%-- el을 사용해서 객체를 지목한 후 메서드를 호출할 때는 get을 제외한 나머지 이름을 얘기하면 됩니다.
