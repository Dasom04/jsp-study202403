...
// 응답할 view를 결정합니다.
@WebServlet("*.do") // 확장자 패턴 -> 앞에 뭐든간에 .do로 끝나는 요청이라면 이 서블릿이 다 받겠다.
public class DancerController extends HttpServlet {

    // 인터페이스 타입의 변수를 선언해서, 상황에 맞게 서비스 객체를 가아 끼울 겁니다.
        ...
        try {
            switch (requestURI) {
                ...
                case "/regist.do":
                    ...
                    // 서비스가 댄서 목록을 repuest에 담아 놓은 상태입니다.
                    // 디스페처에게 목적지를 알려주면서, request와 response객체를 가지고 이동하라는 명령을 내립니다.
                    // 실제 페이지가 목적지로 이동되면서, request와 response객체도 함께 전달됩니다.
                    // -> jsp에서 repuest를 꺼내서 목록을 화면에 부려서 응답하면 끝!
